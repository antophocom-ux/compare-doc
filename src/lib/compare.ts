import { DocumentData, LineItem, ReportData, ComparisonResult, ComparisonDetail, MatchStatus } from '../types';
...
export async function generateReport(rawDocuments: DocumentData[], baseFileName?: string | null): Promise<ReportData> {
  ...
  const baseEmbeddings = await getEmbeddings(baseItemNames);
  ...
  const otherFilesWithEmbeddings = await Promise.all(otherFiles.map(async (file) => {
    ...
    const embeddings = await getEmbeddings(itemNames);
